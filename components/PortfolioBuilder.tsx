    ...
    <div className="space-y-3">
      ...
      {holdings.length === 0 && pending.length === 0 && (
        <p className="text-xs text-ink-muted">
          Add the funds or shares you would have bought. Everything must trade in the same currency —
          conversion between currencies isn&apos;t modelled.
        </p>
      )}
            ...
            >
              ...
              <div className="w-24 shrink-0">
                <NumberInput
                  ...
                  max={100}
                  step={1}
                  ariaLabel={`Weight for ${holding.symbol}`}
                />
              </div>
              <button
                type="button"
      ...
      {holdings.length > 0 && (
        <div className="flex items-center justify-between text-[11px]">
          <span className={weightsOff ? 'text-ink' : 'text-ink-muted'}>
            Total{' '}
            <strong className="tabular font-semibold">{totalWeight.toFixed(1)}%</strong>
            {weightsOff && ' — needs to be 100%'}
          </span>
          <button
            type="button"
            onClick={onEvenSplit}
